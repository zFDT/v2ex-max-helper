  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        finish(null, { statusCode, body, finalUrl: parsed.toString(), headers: res.headers });
      });
    });
    req.on('error', finish);
...
// V2EX 部署在 Cloudflare 后面。托管挑战会以 HTTP 403 返回一个
// "Just a moment..." 页面，并带上 cf-mitigated: challenge。
// 这既不是登录态失效，也不是网络故障：探针无法执行 JS 挑战，重试也不会自愈，
// 必须与 auth_http_status / logged_out 区分开，否则会被误报成「未捕获错误」。
...
function cloudflareChallengeResult(response) {
  const statusCode = (response && response.statusCode) || 0;
  return {
    ok: false,
    code: 'cloudflare_challenge',
    message: `Cloudflare 托管挑战拦截（HTTP ${statusCode || 'unknown'}），当前 UA/IP 被判定为自动化客户端`,
    statusCode,
  };
}

function diagnoseAuthPage(response) {
  if (isCloudflareChallenge(response)) return cloudflareChallengeResult(response);
